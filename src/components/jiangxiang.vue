<template>
  <section style="width: 100%;margin-top: 50px;position: relative;" >
    ...
    <div class="content">
      ...
      <div class="content_right" v-show="contentActive">
        <Form :model="formItem" :label-width="80" style="width: 570px;">
          <FormItem label="名称">
            <Input v-model="formItem.inputName" style="width: 300px;float: left;" placeholder="请输入奖项名称"></Input>
          </FormItem>

          <FormItem label="类型">
			        	<span v-for="(item,index) in prizeType">
							<Button style="width: 68px;margin-right: 10px;margin-bottom: 5px;float: left;" @click="prizeTypeEvent(index)" :class="{'ivu-btn-primary': item.active}">{{item.name}}</Button>
						</span>
          </FormItem>

          <FormItem label="级别">
			        	<span v-for="(item,index) in prizeLevel">
							<Button style="width: 68px;margin-right: 10px;margin-bottom: 5px;float: left;" @click="prizeLevelEvent(index)" :class="{'ivu-btn-primary': item.active}">{{item.name}}</Button>
						</span>
          </FormItem>

          <FormItem label="奖项">
			        	<span v-for="(item,index) in prize">
							<Button style="width: 68px;margin-right: 10px;margin-bottom: 5px;float: left;" @click="prizeEx(index)" :class="{'ivu-btn-primary': item.active}">{{item.name}}</Button>
						</span>
          </FormItem>
    ...
    methods: {
      ...
      prizeEvent(index){
        if(this.alertActive == false){
          ...
          this.$refs.bin.success = this.prizeItem[index].imgId;
        }
      },
      //奖项类型
                    ...
                    for(var k=0;k<this.prize.length;k++){
                      if(this.prize[k].active == true){
                        ...
                        this.$http.post('http://47.106.77.12/hs2f/baoming/saveHuojiang',parameter).then(function(response){
                          ...
                          that.prizeItem[that.prizeRecordIndex].imgId = that.$refs.bin.success;
                          // that.$refs.bin.imgSrc = '';
                          that.info(1);
                        }).catch(function(err){
                          // console.log(err);
      ...
      //预览
      preview(){
        // this.$Message.success('保存奖项成功')
        this.$emit('jiangxiangNext')
      },
      //上一步
      prev(){
        this.$emit('jiangxiangLast')
      },
      //取消
      cancel(){
      ...
      prizeDelete(){
        ...
        this.$http.post('http://47.106.77.12/hs2f/baoming/delHuojiang',parameter).then(function(response){
          that.spin = false;
          // console.log(response)
          that.prizeItem.splice(that.prizeRecordIndex,1);
          that.contentActive = false;
          that.alertActive = false;
          that.addText = '添加奖项';
        }).catch(function(err){
